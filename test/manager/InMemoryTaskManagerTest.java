...
//    9. Убедитесь, что задачи, добавляемые в HistoryManager, сохраняют предыдущую версию задачи и её данных.
    ...
    //    Учитывая, что по условию финального ТЗ 4-го спринта при обновлении данных на вход метода обновления
//    должен подаваться новый объект, который должен полностью заменить старый, то для сохранения в истории предыдущих
//    версий задач достаточно было бы в список истории просмотренных задач добавлять ссылку на объект задачи, который
//    хранится в мапе. Но это справедливо только для объектов Task и Subtask.
//    Состояние эпика же может изменяться не только через метод обновления, но и автоматически, например, при изменении
//    статусов его подзадач. При этом новый объект не создается, ссылка не меняется. Соответственно, при запросе
//    этого эпика до и после автоматического обновления в истории просмотров отобразится в обеих записях новое состояние
//    статуса. Поэтому для корректного отображения истории ВСЕХ просмотров пришлось в методе updateHistory() создавать
//    новый объект Task и копировать значения полей объекта Task, передаваемого на вход метода.
    @Test
